...
const toUrl = (image: SanityImage | null | undefined): string | undefined => {
...
const ProjectYearPage = async ({ params }: ProjectYearPageProps) => {
  ...
  let sanityProject: Awaited<ReturnType<typeof fetchProjectByYear>>;
  try {
    sanityProject = await fetchProjectByYear(year);
  } catch {
    sanityProject = null;
  }
    ...
    ({
      ...
      design: {
        ...
        descentControl: {
          ...
          items: (
            sanityProject.design?.descentControl?.items ?? []
          ).map(
            (item: {
              name?: string;
              bullets?: string[];
      ...
      competitionRequirements: {
        ...
        description:
          sanityProject.competitionRequirements?.description ?? "",
  ...
  const project =
    projectFromSanity || yearObject?.project || null;

  return (
    <ProjectYearContent
      yearLabel={yearObject?.label}
      project={project}
    />
  );
};
